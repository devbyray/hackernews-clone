<template>
	<article class="py-8 space-y-4">
		...
		<h2 class="text-2xl font-medium text-hn-text leading-normal">
			...
			<router-link v-else :to="`/story/${story.id}`" class="hover:text-hn-orange transition-colors">
				{{ story.title }}
			</router-link>
		</h2>
		...
		<div class="flex items-center justify-between pt-2">
			...
			<!-- Add to Reading List -->
			<Button v-if="showReadingList" variant="ghost" class="flex items-center space-x-2">
				<svg class="w-6 h-6 text-hn-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path
						stroke-linecap="round"
						stroke-linejoin="round"
